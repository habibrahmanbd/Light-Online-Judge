...
#define p_z 500002 //prime size need
ll prime[p_z/10];
...
//    _F_out;
    ...
    for(int cs=1; cs<=t; cs++)
    {
        ...
        sort(arr+1, arr+n+1);
        init(500005, 500005);//cout<<"Pro";
        for(int i=1; i<=n; i++)
        {
            for(int j=0; j<total; j++)
            {
                if(arr[i]*prime[j]>arr[n])
                    break;
                if(_map[arr[i]*prime[j]])
                    addEdge(arr[i], (arr[i]*prime[j])),
                            addEdge((arr[i]*prime[j]),arr[i]);

            }
        }
        int r1 = Hopcraft_Karp()/2;
        //cout<<r1<<endl;
        int res = (n - (r1*2))+r1;
        pf("Case %d: %d\n",cs, res);
