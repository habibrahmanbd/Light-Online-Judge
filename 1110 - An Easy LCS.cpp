...
//------------------------------------------------------
//IT'S GIVING ME WRONG ANS
string str1, str2;
string ans_str[102][102];
ll len1, len2; //length of string.
ll dp[102][102]; //stores the maximum length of LCS
    ...
    for(ll id1=1; id1<=len1; id1++)
    {
        for(ll id2=1; id2<=len2; id2++)
        {
            if(str1[id1-1]==str2[id2-1])
            {
                ...
                ans_str[id1][id2] = ans_str[id1-1][id2-1]+str1[id1-1];
            }
            else if(dp[id1-1][id2]>dp[id1][id2-1])
            {
                dp[id1][id2]=dp[id1-1][id2];
                ans_str[id1][id2] = ans_str[id1-1][id2];
            }
            else if(dp[id1-1][id2]<dp[id1][id2-1])
            {
                dp[id1][id2]=dp[id1][id2-1];
                ans_str[id1][id2] = ans_str[id1][id2-1];
            }
            else
            {
                dp[id1][id2]=dp[id1-1][id2];
                ans_str[id1][id2]=min(ans_str[id1-1][id2], ans_str[id1][id2-1]);
            }
        }
    }
    return;
