...
#define _F_in       freopen("1037 - Agent 47.IN","r",stdin)
#define _F_out      freopen("1037 - Agent 47.OUT","w",stdout)
...
ll dp_rec(ll count, ll mask)
{
    ...
    for(ll i=0; i<n; i++)
    {
        if(!Check(mask, i))
        {
            for(ll j=0; j<=n; j++)
            {
                if(Check(mask, j)&&power[j][i])
                {
                    mask = Set(mask, i);
                    res = (ll)ceil((db)Health[i]/(db)power[j][i]);
                    ans = min(ans, res+dp_rec(count+1,mask));
                    mask = Reset(mask, i);
                }
            }
        }
    ...
    for(ll cs=1; cs<=T; cs++)
    {
        ...
        for(ll i=0; i<n; i++)
        {
            ...
            for(ll j=0; j<str.length(); j++)
                power[i][j]=(ll)cdigittoint(str[j]);
        }
        mask=0;
        mask=Set(mask, n);
        for(ll i=0; i<n ; i++)
