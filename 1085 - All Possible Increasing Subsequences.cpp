...
#define _F_in       freopen("1085.in","r",stdin)
#define _F_out      freopen("1085.out","w",stdout)
...
int main()
{
    ...
    for(ll cs=1; cs<=t; cs++)
    {
        ...
        for(ll i=1; i<=N; i++)
        {
            ll TOT = LST[arr[i]]; //The number of element less than arr[i] +1
            ll VAL = Query(TOT-1)+1;
            Res = (Res + VAL)%mod;
            Update(TOT, VAL);
        }
        pf("Case %lld: %lld\n",cs, Res);
