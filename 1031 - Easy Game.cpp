...
#define _F_in       freopen("1031 - Easy Game.in","r",stdin)
#define _F_out      freopen("1031 - Easy Game.out","w",stdout)
...
ll dp_rec(ll start, ll end)
{
    ...
    {
        sum+=arr[i];
        diff = max(diff, sum-dp_rec(i+1, end));
    }
    sum=0;
    for(ll i=end; i>=start; i--)
    {
        sum+=arr[i];
        diff = max(diff, sum-dp_rec(start, i-1));
    }
