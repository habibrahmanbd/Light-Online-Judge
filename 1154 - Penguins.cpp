...
#define MAX_V 205
ll x[MAX_V], y[MAX_V], peng[MAX_V], jump[MAX_V];
ll cost[MAX_V][MAX_V], mf, f, s, t;
bool cnt[MAX_V][MAX_V];                       // global variables
vll p;
vector<vll> AdjList;
...
void augment(ll v, ll minEdge)       // traverse BFS spanning tree from s to t
{
    ...
    {
        augment(p[v], min(minEdge, cost[p[v]][v])); // recursive
        cost[p[v]][v] -= f;
        cost[v][p[v]] += f;
    }       // update
}
...
int main()
{
    ...
    for(ll cs=1; cs<=t; cs++)
    {
        ...
        ll totalPeng=0;
        for(ll i=1; i<=N; i++)
            x[i]=ILL, y[i]=ILL, peng[i]=ILL, jump[i]=ILL,totalPeng+=peng[i];
        //cout<<totalPeng<<endl;

        clr(cnt,0);
        for(ll i=1; i<=N; i++)
            for(ll j=1; j<=N; j++)
                if(f_dist(x[i],y[i], x[j],y[j])<=mx_dst)
                    cnt[i][j]=cnt[j][i]=1;

        s=N+N+1;
        vll res;
        AdjList.assign(s+5, vll());
        for(ll i=1; i<=N; i++)
        {
            AdjList[i].pb(s);
            AdjList[s].pb(i);
            ...
            for(ll j=1; j<=N; j++)
                if(cnt[i][j])
                    AdjList[i+N].pb(j),AdjList[j].pb(i+N);
        }
        //cout<<"HI";
        for(ll nd=1; nd<=N; nd++)
        {
