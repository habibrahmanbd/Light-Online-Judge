...
#define _F_in       freopen("1083- Histogram .in","r",stdin)
#define _F_out      freopen("1083- Histogram .out","w",stdout)
...
void Init(ll node, ll beg, ll en)
{
    ...
    Init(left, beg, mid);
    Init(right, mid+1, en);
//    Tree[node].Data=min(Tree[left].Data, Tree[right].Data);
    if(Tree[left].Data<Tree[right].Data)
    {
        Tree[node].Pos=Tree[left].Pos;
        Tree[node].Data=Tree[left].Data;
    }
    else
    {
        Tree[node].Pos=Tree[right].Pos;
        Tree[node].Data=Tree[right].Data;
    }
}

Node Query(ll node, ll beg, ll en, ll i, ll j)
{
    if(i>en||j<beg)
        return InF;
    else if(beg>=i&&en<=j)
    ...
    Node p2 = Query(right, mid+1, en, i, j);
    if(p1.Data<=p2.Data)
        return p1;
    else
        return p2;
...
ll Solve(ll Beg, ll En)
{
    if(Beg<=En)
    {
        Node Pos_of_Min = Query(1, 1, N, Beg, En);
        if(Beg==En)
            return Pos_of_Min.Data;

//    cerr<<"Hello";
        ll Len = En-Beg+1;
        ll Res1 = Len*arr[Pos_of_Min.Pos];
        ll Res2 = Solve(Beg, Pos_of_Min.Pos-1);
        ll Res3 = Solve(Pos_of_Min.Pos+1, En);
