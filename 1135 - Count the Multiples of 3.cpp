...
#define _F_in       freopen("1135.in","r",stdin)
#define _F_out      freopen("1135.out","w",stdout)
...
void Build(ll Idx, ll Beg, ll En, ll St_Po, ll En_Po)
{
    ll Left = Idx<<1;
    ll Right = Left+1;
    ll Mid = (Beg+En)>>1;

    if(Beg>En_Po||En<St_Po)
        return;

    if(Beg==En)
    {
        Tree[Idx].propagate=0, Tree[Idx].rem[0]=1,Tree[Idx].rem[1]=0,Tree[Idx].rem[2]=0;
        return;
    }
    Build(Left, Beg, Mid, St_Po, En_Po);
    Build(Right, Mid+1, En, St_Po, En_Po);

    Tree[Idx].rem[0] = Tree[Left].rem[0]+Tree[Right].rem[0];
//    cout<<Tree[Idx].rem[0]<<endl;
    Tree[Idx].rem[1] = Tree[Left].rem[1]+Tree[Right].rem[1];
    Tree[Idx].rem[2] = Tree[Left].rem[2]+Tree[Right].rem[2];
    return;
}

void Update(ll Idx, ll Beg, ll En, ll St_Po, ll En_Po, ll Propagate)
{
    ll Left = Idx<<1;
    ll Right = Left+1;
    ll Mid = (Beg+En)>>1;
    if(Tree[Idx].propagate%3>0)
    {
        ll temp=Tree[Idx].propagate%3;
        if(temp==1)
        {
            ll ss = Tree[Idx].rem[2];
            Tree[Idx].rem[2]=Tree[Idx].rem[1];
            Tree[Idx].rem[1]=Tree[Idx].rem[0];
            Tree[Idx].rem[0]=ss;
        }
        else if(temp==2)
        {
            ll ss = Tree[Idx].rem[2];
            Tree[Idx].rem[2]=Tree[Idx].rem[0];
            Tree[Idx].rem[0]=Tree[Idx].rem[1];
            Tree[Idx].rem[1]=ss;
        }
    ...
    if(Beg>=St_Po&&En<=En_Po)
    {
        Tree[Idx].propagate+=Propagate;
        Propagate=0;
        ll temp = Tree[Idx].propagate%3;
    ...
    Update(Left, Beg, Mid, St_Po, En_Po, Propagate);
    Update(Right, Mid+1, En, St_Po, En_Po, Propagate);
...
void Query(ll Idx, ll Beg, ll En, ll St_Po, ll En_Po)
{
    ll Left = Idx<<1;
    ll Right = Left+1;
    ll Mid = (Beg+En)>>1;
    ...
    Query(Left, Beg, Mid, St_Po, En_Po);
    Query(Right, Mid+1, En, St_Po, En_Po);
    return;
}
...
int main()
{
    ...
    for(ll cs=1; cs<=T; cs++)
    {
        pf("Case %lld:\n",cs);
        ll N=ILL, Q=ILL;
        for(ll Idx=0; Idx<4*MaxN; Idx++)
        Tree[Idx].propagate=0, Tree[Idx].rem[0]=0,Tree[Idx].rem[1]=0,Tree[Idx].rem[2]=0;

        Build(1, 1, N, 1, N);
        for(ll i=1; i<=Q; i++)
        {
            res=0;
            ll ty=ILL, fm=ILL, to=ILL;
            fm++, to++;
            if(ty==0)
            {
                Update(1, 1, N, fm, to, 1);
            }
            else
            {
                Query(1, 1, N, fm, to);
