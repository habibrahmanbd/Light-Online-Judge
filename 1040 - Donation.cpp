...
vi adj[60];
int total_sure, total_with_out_sure;
bool vis[60];
void normal_dfs(int u)
{
    vis[u]=1;
        ...
        for(int i=0; i<n; i++)
        {
            for(int j=0; j<n; j++)
            {
                ...
                if(co>0)
                {
                    ...
                    {
                        total_with_out_sure+=co;
                        AdjList[i].pb(mp(j,co));
                        AdjList[j].pb(mp(i,co));
                        adj[i].pb(j);
                        adj[j].pb(i);
                    }
                }
...
//        printf("MST cost = %d (Prim's)\n", mst_cost);
        pf("Case %d: %d\n",cs,total_sure+(total_with_out_sure-mst_cost));
//        cout<<total_sure<<endl;
