...
#define _F_in       freopen("1257 - Farthest Nodes in a Tree (II).IN","r",stdin)
#define _F_out      freopen("1257 - Farthest Nodes in a Tree (II).OUT","w",stdout)
...
vector<pair<ll, ll> >adjList[30005];
ll n;
ll dist[2][30005];
ll Max_dist=-1;
ll Max_point ;
...
void BFS(ll src, ll ptr)
{
    ...
    while(!Q.empty())
    {
        ...
        for(ll i=0; i<adjList[U].size(); i++)
        {
            ...
            if(!vis[ptr][V])
            {
                dist[ptr][V] = dist[ptr][U]+adjList[U][i].sc;
                Q.push(V);
                vis[ptr][V]=1;
            }
        }
