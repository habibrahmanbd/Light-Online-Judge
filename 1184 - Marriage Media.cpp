...
int main()
{
    ...
    for(int cs=1; cs<=t; cs++)
    {
        ...
        for(int i=1; i<=m; i++)
        {
            for(int j=1; j<=n; j++)
            {
                if((abs(men[i].heigth-women[j].heigth)<=12)&&(abs(men[i].age-women[j].age)<=5)&&(men[i].is_divorse==women[j].is_divorse)) //valid check
                {
                    addEdge(i, j+m);
                    addEdge(j+m, i);
                }
