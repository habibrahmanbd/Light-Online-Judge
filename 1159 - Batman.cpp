...
#define _F_in       freopen("1159 - Batman.in","r",stdin)
#define _F_out      freopen("1159 - Batman.out","w",stdout)
...
ll dp_rec(ll i, ll j, ll k)
{
    ...
    {
        dp[i][j][k] = 1 + dp_rec(i+1,j+1,k+1);
    }
    else
    {
        ll v1 = dp_rec(i,j,k+1);
        ll v2 = dp_rec(i,j+1,k);
        ll v3 = dp_rec(i,j+1,k+1);
        ll v4 = dp_rec(i+1,j,k);
        ll v5 = dp_rec(i+1,j,k+1);
        ll v6 = dp_rec(i+1,j+1,k);
        ll mx = max(max(max(max(max(v1,v2),v3),v4),v5),v6);
        dp[i][j][k]=mx;
    }
    vis[i][j][k]=1;
